...
# 本项目通过工厂模式创建LLM实例，根据 LLM_PROVIDER 环境变量
# 自动选择 OpenAI / Qwen / Ollama 后端 (详见 src/llm/factory.py)
...
messages = [
    SystemMessage(content="You are a smart home assistant."),
    HumanMessage(content="Turn on the living room light.")
]
...
# 注意: 实际代码中 Prompt 使用英文编写，以获得更稳定的 JSON 解析结果
...
2. **rooms**: List of rooms mentioned or implied
3. **actions**: Specific actions mentioned
4. **mood**: Mood/atmosphere if mentioned
5. **scene_hint**: If a predefined scene is referenced
6. **constraints**: Any constraints mentioned
...
Given the user's intent and available devices, generate a list of specific
device commands.

Rules:
1. Only use devices listed in the context
2. Only use capabilities the device actually has
3. Include a brief reason for each action
4. Reference matching scenes but adjust as needed
...
下一节：[05_LangGraph工作流.qmd](./05_LangGraph工作流.qmd)
