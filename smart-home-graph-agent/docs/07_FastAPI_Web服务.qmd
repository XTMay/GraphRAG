...
**Pydantic 的三个作用：** 1. **类型验证** — 自动拒绝格式错误的请求（返回 422） 2. **文档生成** — 自动生成 JSON Schema → Swagger UI 展示 3. **序列化** — 自动将 Python 对象转为 JSON 响应
...
**为什么用依赖注入？** - 端点函数不需要知道 Agent 如何创建 - 测试时可以替换为 Mock Agent - 资源初始化失败时统一返回 503
...
**设计要点：** - **版本控制**：`/api/v1/` 前缀，方便未来升级到 v2 - **资源命名**：使用名词（`rooms`、`devices`），不用动词 - **嵌套资源**：`/rooms/{name}/devices` 表示房间包含设备
...
下一节：[08_Docker容器化部署.qmd](./08_Docker容器化部署.qmd)